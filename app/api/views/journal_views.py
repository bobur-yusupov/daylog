                        ...
                        "blocks": [
                            {
                                "id": "header-1",
                                "type": "header",
                                "data": {"text": "Today's Reflection", "level": 2},
                            },
                            {
                                "id": "paragraph-1",
                                "type": "paragraph",
                                "data": {"text": "Today was a productive day..."},
                            },
                        ],
                        "version": "2.28.2",
                    },
...
class JournalEntryViewSet(viewsets.ModelViewSet):
    ...
    def search(self, request):
        ...
        return Response(
            {"query": query, "results": serializer.data, "count": queryset.count()}
        )
            ...
            if isinstance(content, dict) and "blocks" in content:
                ...
                for block in content["blocks"]:
                    ...
                    stats["block_types"][block_type] = (
                        stats["block_types"].get(block_type, 0) + 1
                    )
    ...
    def filtered(self, request):
        ...
        # Note: min_words and max_words could be implemented in the future for word count filtering
    ...
    def public(self, request):
        ...
        queryset = (
            JournalEntry.objects.filter(is_public=True)
            .select_related("user")
            .prefetch_related("tags")
            .order_by("-created_at")
        )
        ...
        # Fallback if pagination is not configured
        serializer = self.get_serializer(queryset, many=True)
        return Response({"entries": serializer.data, "count": queryset.count()})
