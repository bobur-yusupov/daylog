...
class EmailVerificationViewTests(TestCase):
    ...
    def test_post_verify_email_invalid_code(self):
        ...
        form_data = {'otp_code': '999999'}  # Wrong code, verification has different code
        response = self.client.post(self.verify_email_url, form_data)
        ...
        if messages:
            ...
            has_error_message = any('Invalid' in text or 'check' in text for text in message_texts)
        else:
            # If no messages, check if form has errors (alternative error display)
            form = response.context.get('form')
            has_error_message = form and not form.is_valid()
        
        self.assertTrue(has_error_message, "Should have error message or form errors for invalid OTP")
    ...
    def test_post_verify_email_form_validation_errors(self):
        ...
        self.assertIn('Ensure this value has at least 6 characters', str(form.errors['otp_code']))
...
class ResendOTPViewTests(TestCase):
    ...
    def test_post_resend_otp_failure(self, mock_resend):
        ...
        # Mock resend failure
        from authentication.services.email_verification_service import EmailVerificationResult
        mock_resend.return_value = EmailVerificationResult(success=False, error_message="Email sending failed")
...
class EmailVerificationFlowIntegrationTests(TestCase):
    ...
    def test_multiple_verification_attempts(self):
        ...
        another_verification = EmailVerification.objects.create(
            user=new_user,
            otp_code=verification.otp_code  # Same code
        )
