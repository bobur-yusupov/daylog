...
class EmailVerificationService:
    ...
    def send_verification_email(user) -> EmailVerificationResult:
        ...
        try:
            ...
            logger.exception(
                "Database error while creating verification", extra={"user_id": user.id}
            )
            return EmailVerificationResult(
                success=False,
                error_message="Database error during verification creation",
            )
            ...
            return EmailVerificationResult(
                success=False, error_message="Unexpected error during email sending"
            )
    ...
    def verify_email_with_otp(user, otp_code: str) -> EmailVerificationResult:
        try:
            ...
            if not verification:
                return EmailVerificationResult(
                    success=False, error_message="Invalid or expired verification code"
                )
            ...
            return EmailVerificationResult(
                success=False, error_message="Database error during verification"
            )

        except Exception:
            logger.exception("Unexpected verification error", extra={"user": user.id})
            return EmailVerificationResult(
                success=False, error_message="Unexpected error during OTP verification"
            )
    ...
    def resend_verification_email(user) -> EmailVerificationResult:
        ...
        try:
            ...
            return EmailVerificationResult(
                success=False, error_message="Database error during resend"
            )

        except Exception:
            logger.exception("Unexpected resend error", extra={"user": user.id})
            return EmailVerificationResult(
                success=False, error_message="Unexpected error during resend"
            )
    ...
    def _render_email_templates(context):
        html_content = render_to_string(
            EmailVerificationService.OTP_TEMPLATE_HTML, context
        )
        text_content = render_to_string(
            EmailVerificationService.OTP_TEMPLATE_TEXT, context
        )
        return html_content, text_content

    @staticmethod
    def _deliver_email(
        subject: str, text_content: str, html_content: str, recipient: str
    ) -> None:
    ...
    def _send_otp_email(user, verification) -> None:
        ...
        try:
            html_content, text_content = (
                EmailVerificationService._render_email_templates(context)
            )
            EmailVerificationService._deliver_email(
                subject, text_content, html_content, user.email
            )

        except TemplateDoesNotExist as e:
            logger.error("Email template not found: %s", e)
            raise TemplateRenderingError(
                "Email template missing or misconfigured"
            ) from e
