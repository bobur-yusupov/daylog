...
class JournalModelValidationTests(TestCase):
    ...
    def test_foreign_key_validation(self):
        ...
        # Test foreign key constraint - this is more of a database integrity test
        # For practical purposes, we'll test that a valid FK works
        entry_valid = JournalEntry(
            user=self.user,  # Valid user
            title='Valid FK Test',
            content=self.sample_content
        )
        
        # This should work without issues
        entry_valid.full_clean()
        entry_valid.save()
        
        # Verify it was saved
        self.assertTrue(JournalEntry.objects.filter(title='Valid FK Test').exists())
    ...
    def test_unique_constraint_validation(self):
        ...
        # Create first tag
        tag1 = Tag.objects.create(user=self.user, name='UniqueTest')
...
class JournalModelBusinessLogicTests(TestCase):
    ...
    def test_model_field_properties(self):
        ...
        # Name field is no longer globally unique, but has unique_together constraint
