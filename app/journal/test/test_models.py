...
class JournalEntryModelUnitTests(TestCase):
            ...
            "blocks": [
                {
                    "id": "header-1",
                    "type": "header",
                    "data": {"text": "My Journal Entry", "level": 2},
                },
                {
                    "id": "paragraph-1",
                    "type": "paragraph",
                    "data": {"text": "This is my first journal entry content."},
                },
            ],
            "version": "2.28.2",
        }
    ...
    def test_journal_entry_str_representation(self):
        ...
        entry = JournalEntry.objects.create(
            user=self.user1, title="Test Entry", content=self.sample_content
        )
    ...
    def test_journal_entry_title_max_length(self):
        ...
        with self.assertRaises(ValidationError):
            entry = JournalEntry(
                user=self.user1, title=long_title, content=self.sample_content
            )
            entry.full_clean()
    ...
    def test_journal_entry_null_fields(self):
        ...
        entry = JournalEntry.objects.create(
            user=self.user1, title="", content=self.sample_content  # Empty string
        )
    ...
    def test_journal_entry_json_content(self):
        ...
        entry = JournalEntry.objects.create(
            user=self.user1, title="JSON Test", content=self.sample_content
        )
    ...
    def test_journal_entry_timestamps_auto_update(self):
        ...
        entry = JournalEntry.objects.create(
            user=self.user1, title="Test", content=self.sample_content
        )
...
class TagJournalEntryIntegrationTests(TestCase):
            ...
            "blocks": [
                {
                    "id": "paragraph-1",
                    "type": "paragraph",
                    "data": {"text": "This is a test journal entry."},
                }
            ],
            "version": "2.28.2",
        }
    ...
    def test_entry_without_tags(self):
        ...
        entry = JournalEntry.objects.create(
            user=self.user, title="No Tags Entry", content=self.sample_content
        )
    ...
    def test_tag_filtering_by_entries(self):
        ...
        common_tags = Tag.objects.filter(journalentry__in=[entry1, entry2]).distinct()
...
class ModelValidationIntegrationTests(TestCase):
    ...
    def test_complex_json_content_validation(self):
            ...
            "blocks": [
                {
                    "id": "header-1",
                    "type": "header",
                    "data": {"text": "Complex Entry", "level": 1},
                },
                {
                    "id": "list-1",
                    "type": "list",
        ...
        entry = JournalEntry.objects.create(
            user=self.user, title="Complex Entry", content=complex_content
        )

        entry.refresh_from_db()
        self.assertEqual(
            entry.content["blocks"][1]["data"]["items"], ["Item 1", "Item 2", "Item 3"]
        )
        self.assertEqual(len(entry.content["blocks"]), 3)

    def test_edge_case_empty_content(self):
        """Test handling of empty or minimal content"""
        empty_content = {"time": 1643723964077, "blocks": [], "version": "2.28.2"}

        entry = JournalEntry.objects.create(
            user=self.user, title="Empty Entry", content=empty_content
        )
    ...
    def test_unicode_content_handling(self):
            ...
            "blocks": [
                {
                    "id": "paragraph-1",
                    "type": "paragraph",
                    "data": {"text": "Unicode test: 你好世界 🌍 café naïve résumé"},
                }
            ],
            "version": "2.28.2",
        }
        ...
        entry = JournalEntry.objects.create(
            user=self.user, title="Unicode Test Entry 测试", content=unicode_content
        )
